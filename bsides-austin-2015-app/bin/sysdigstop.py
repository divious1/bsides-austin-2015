...
def sysdigstop(endpoint):
    
    if process_name:
        ssh = subprocess.Popen(['ssh', '-o StrictHostKeyChecking=no', '-i{0}'.format(SSH_KEY), '{0}@{1}'.format(USER,endpoint), 'sudo killall', 'sysdig'], shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
if __name__ == '__main__':
    ...
    try:
        ...
        for entry in results:
            ...
            #kill process
            sysdigstop(endpoint)
            logger.warn('sent process kill command for sysdig to host {0})'.format(endpoint))
            break

    except Exception as e:
        logger.error("There was an issue establishing arguments for the " +
                     "sysdigstop earch command!")
